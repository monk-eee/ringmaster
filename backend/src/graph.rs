...
pub enum UpsertNodesError {
    InvalidInput(String),
    AmbiguousIdentity { node_type: String, canonical_text: String, matches: usize },
    Database(sqlx::Error),
}
...
pub async fn list_nodes(
    ...
    list_nodes_filtered(pool, node_type, None, occurred_from, occurred_to, needs_attention, limit, offset).await
}
...
pub async fn upsert_nodes(pool: &PgPool, entities: Vec<EntityUpsert>) -> Result<Vec<EntityUpsertResult>, UpsertNodesError> {
    ...
    for entity in entities {
        ...
        if node_type.is_empty() {
            return Err(UpsertNodesError::InvalidInput("node_type must not be blank".to_string()));
        }
        if canonical_text.is_empty() {
            return Err(UpsertNodesError::InvalidInput("canonical_text must not be blank".to_string()));
        }
        if entity.lifecycle_state.as_deref().is_some_and(|value| value.trim().is_empty()) {
            return Err(UpsertNodesError::InvalidInput("lifecycle_state must not be blank when supplied".to_string()));
        }
        if entity.attributes.as_ref().is_some_and(|value| !value.is_object()) {
            return Err(UpsertNodesError::InvalidInput("attributes must be a JSON object when supplied".to_string()));
        }
        ...
        results.push(EntityUpsertResult { action: action.to_string(), node });
    }
...
pub async fn list_edges_for_node(pool: &PgPool, id: Uuid, edge_type: Option<&str>, current_only: bool) -> Result<Vec<Edge>, sqlx::Error> {
        sqlx::query_as(
