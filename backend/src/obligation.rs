...
/// ADR-0093: an Obligation was 100% read-only across every surface (HTTP,
/// CLI, MCP, UI) before this -- status could only ever be set at creation.
/// Rejected before any row is written, mirroring `AppendEventError`.
#[derive(Debug)]
pub enum UpdateObligationError {
    NotFound,
...
impl fmt::Display for UpdateObligationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NotFound => write!(f, "obligation not found"),
            Self::InvalidStatus(reason) => write!(f, "{reason}"),
            Self::NoChange => write!(
                f,
                "an update must change status, hard_due_at, or soft_due_at"
            ),
            Self::Database(error) => write!(f, "{error}"),
        }
    }
...
impl std::error::Error for UpdateObligationError {}

/// Updates an existing Obligation's status and/or due dates by appending a
/// `status_changed` event (ADR-0093) -- the event log stays authoritative;
/// this never patches `obligation_projection` directly, matching every
/// other write in this module. Shared by the HTTP route
/// (`api/obligations.rs`), the `update-obligation` CLI subcommand, and the
/// `update_obligation` MCP tool so none of the three can drift from the
/// others' semantics.
///
/// `hard_due_at`/`soft_due_at`: `None` leaves the field unchanged (omitted
/// from the event payload entirely, so `rebuild_projection` carries the
/// prior value forward); `Some("")` explicitly clears it (an empty string
/// fails RFC3339 parsing, the same "present but not parseable clears it"
/// contract `payload_timestamp` already uses); `Some(<rfc3339>)` sets it.
/// `actor`/`source` are recorded on the audit row so the trail honestly
/// distinguishes which surface made the change (e.g. `"http_api"`, `"cli"`,
/// `"mcp"`).
pub async fn update_status(
    pool: &PgPool,
    obligation_id: Uuid,
    new_status: Option<ObligationStatus>,
    hard_due_at: Option<String>,
    soft_due_at: Option<String>,
    actor: &str,
    source: &str,
) -> Result<ObligationProjection, UpdateObligationError> {
    let current: ObligationProjection = sqlx::query_as(
        "SELECT obligation_id, status, hard_due_at, soft_due_at, source_fragment_id \
         FROM obligation_projection WHERE obligation_id = $1",
    )
    .bind(obligation_id)
    .fetch_one(pool)
    .await
    .map_err(|error| match error {
        sqlx::Error::RowNotFound => UpdateObligationError::NotFound,
        other => UpdateObligationError::Database(other),
    })?;

    if new_status.is_none() && hard_due_at.is_none() && soft_due_at.is_none() {
        return Err(UpdateObligationError::NoChange);
    }

    let effective_status = new_status.unwrap_or_else(|| {
        ObligationStatus::parse(&current.status)
            .expect("obligation_projection always stores a status update_status recognizes")
    });

    let mut payload = serde_json::Map::new();
    payload.insert(
        "status".to_string(),
        Json::String(effective_status.as_str().to_string()),
    );
    if let Some(value) = &hard_due_at {
        payload.insert("hard_due_at".to_string(), Json::String(value.clone()));
    }
    if let Some(value) = &soft_due_at {
        payload.insert("soft_due_at".to_string(), Json::String(value.clone()));
    }

    let mut tx = pool.begin().await.map_err(UpdateObligationError::Database)?;
    append_event(
        &mut *tx,
        obligation_id,
        ObligationEventType::StatusChanged,
        Json::Object(payload.clone()),
    )
    .await
    .map_err(|error| match error {
        AppendEventError::InvalidPayload(reason) => UpdateObligationError::InvalidStatus(reason),
        AppendEventError::Database(error) => UpdateObligationError::Database(error),
    })?;
    crate::audit::record(
        &mut *tx,
        actor,
        ...
        Some(serde_json::json!({
            ...
            "hard_due_at": current.hard_due_at,
            "soft_due_at": current.soft_due_at,
        })),
    ...
    .await
    .map_err(UpdateObligationError::Database)?;
    tx.commit().await.map_err(UpdateObligationError::Database)?;
...
mod tests {
    ...
    /// ADR-0093: the first edit surface an Obligation has ever had.
    #[tokio::test]
    async fn update_status_changes_status_and_carries_forward_due_dates() {
        let pool = test_pool().await;
        let obligation_id = Uuid::new_v4();
        append_event(
            &pool,
        ...
        assert!(
            updated.hard_due_at.is_some(),
            "omitting hard_due_at must carry the previously-recorded value forward, not clear it"
        );
    }

    #[tokio::test]
    async fn update_status_can_explicitly_clear_a_due_date() {
        let pool = test_pool().await;
        let obligation_id = Uuid::new_v4();
        append_event(
            &pool,
        ...
        assert_eq!(
            updated.hard_due_at, None,
            "an explicit empty string must clear hard_due_at"
        );
    }

    #[tokio::test]
    async fn update_status_rejects_a_no_op_update() {
        let pool = test_pool().await;
        let obligation_id = Uuid::new_v4();
        append_event(
            &pool,
        ...
        let result = update_status(&pool, obligation_id, None, None, None, "test-actor", "test").await;
        assert!(matches!(result, Err(UpdateObligationError::NoChange)));
    }

    #[tokio::test]
    async fn update_status_rejects_an_unknown_obligation() {
        let pool = test_pool().await;
        let result = update_status(
            &pool,
            Uuid::new_v4(),
            Some(ObligationStatus::Closed),
            None,
            None,
            "test-actor",
            "test",
        ...
        .await;
        assert!(matches!(result, Err(UpdateObligationError::NotFound)));
    }
}
