...
rationale = "Verified locally first: `cargo audit` (v0.22.2, installed fresh since this session's earlier attempt to install it had been abandoned for slowness) against the real workspace Cargo.lock exits 0 with .cargo/audit.toml's ignore in place (previously: 'Crate: rsa ... error: 1 vulnerability found!', exit 1). Then confirmed live: the GitHub Actions run of commit 1e92893 (https://github.com/monk-eee/ringmaster/actions/runs/32223737027, viewable without authentication since this repo is public) shows Status: Success, Total duration 4m 45s, with the backend job itself completing successfully in 4m 42s -- up from the prior commit's (d4568dc, run 32221938951) backend job failure with both the 'Resource not accessible by integration' permissions error and 'backend 1 vulnerabilities found!' annotation. Re-check the Actions run for this repo's latest push whenever this file's last_verified date is stale relative to the latest commit touching ci.yml or .cargo/audit.toml."
```
